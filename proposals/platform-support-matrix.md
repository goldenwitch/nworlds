...
| Arch Linux / SteamOS-family desktop (`x86_64-unknown-linux-gnu`) | `winit` Wayland/X11 lifecycle; `wgpu` surface, device, queue, resize, and shutdown | Native events -> `PlatformInputAdapter` -> `InputPacket` | Target ingress adapter | `WgpuRenderSink`; Vulkan backend preferred, fallback measured | Anchor codec | Target transport TBD | Arch-compatible build CI plus native Wayland runtime evidence | gap: no Arch/SteamOS-family lane |
