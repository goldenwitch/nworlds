use caravan_domain::{Actor, ActorId, ActorKind, Effect, Terrain, TileId, TileLayers, Resources};
use caravan_vegetation::{
    Forest, Forester, IndexedInput, IndexedTile, Farmer, FarmerResult, Snapshot, VegetationSnapshot,
    Wheat,
};
...
fn farmer_uses_fixed_order_disappears_and_places_wheat_around_destination() {
    ...
    assert_eq!(snapshot.layers_at(tile(0, 0)).expect("origin layer").terrain(), Terrain::Void);
}
...
fn forester_stays_on_forest_and_wood_is_an_indexed_total() {
    ...
    let snapshots = [
        Snapshot::new(
            0,
            [indexed_tile(tile(0, 0), Terrain::Forest, Some(forester.id()), Effect::None)],
            [forester],
        ),
        Snapshot::new(
            1,
