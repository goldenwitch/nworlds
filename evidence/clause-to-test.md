...
same cases and writes machine-readable results. The existing trace integration
test additionally checks the complete stdout trace against
`crates/caravan-sample/snapshots/anchor-trace.txt`.
...
| Runnable anchor observables | `caravan-trace` | `anchor-trace.txt` and trace snapshot test | Runtime pass |
...
| Clause | Test | Evidence | Coverage |
| --- | --- | --- | --- |
| State-aware interaction and explicit projection failure | `identical_input_and_tau_use_the_selected_logical_state`, `malformed_selected_state_fails_before_interaction` | `crates/caravan-sample/tests/input.rs` | Root workspace runtime pass |
| Ordered transport batch to semantic interaction | `ordered_transport_batch_derives_the_current_membership_interaction_view` | `crates/caravan-sample/tests/input.rs` | Runtime pass; identity/order normalize into the payload-only semantic batch while the membership view remains compatible |
| Orchestrator input-window lifecycle | `input_buffer_snapshots_and_resolves_only_the_captured_window`, `retained_window_survives_an_interaction_resolution`, `empty_input_is_a_noop_and_branch_append_requires_explicit_policy` | `crates/caravan-sample/src/input.rs`; `crates/caravan-sample/src/orchestrator.rs` | Runtime pass; later arrivals remain pending, successful/no-op windows resolve, and rejected publication retains input |
| In-memory host crossing | `host_crossing_delivers_input_publishes_immutable_state_and_collects_frame`, `input_transport_order_is_preserved_before_semantic_batch_conversion`, `host_storage_round_trip_preserves_selected_worldline_and_scrubbing` | `crates/caravan-sample/tests/host.rs` | Runtime pass; independent input, render, storage, publication, and scrubbing crossings remain observable |
...
| Caravan remaps onto the generic desktop composition | `cargo check -p caravan-sample --all-targets --locked` | Compile pass: the sample-owned `CaravanInputAdapter` and `CaravanPackage` compose through `nworlds-desktop::DesktopApplication`; native runtime observation remains separate |
...
| Current Caravan client launches through the generic desktop composition | `cargo run -p caravan-sample --locked` | Manual local runtime smoke: the sample composition opened without startup errors on Windows; no device-support claim |
...
| Both samples use the driver-backed presentation path | `cargo test -p caravan-sample --locked`; `cargo test -p voxel-sample --locked` | Caravan `present_state` and voxel `VoxelPackage` presentation paths remain green with immutable worldline tests |
