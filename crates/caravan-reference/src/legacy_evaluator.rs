    ...
    fn apply_entry(&mut self, entry: &JournalEntry<GameJournalEntry>, context: ReferenceContext) {
        match *entry.payload() {
            GameJournalEntry::CreateSaucer { radius }
                if radius == context.saucer_radius() && self.saucer.is_none() =>
            {
...
mod tests {
    ...
    fn journal() -> crate::Journal {
        ...
        writer.record(GameJournalEntry::SetTerrain {
            tile: TileId::new(1, 0).expect("fixture tile is inside the saucer"),
            terrain: Terrain::Forest,
        });
        writer.record(GameJournalEntry::SpawnActor {
            id: ActorId::new(1).expect("fixture actor ID is positive"),
    ...
    #[test]
    fn legacy_fold_matches_projection_on_shared_fixture() {
        let context = context();
        let journal = journal();

        for ticks in [-1, 0, 499, 500, 999, 1_000, 2_000] {
            let logical_time = LogicalTime::from_ticks(ticks);
            let legacy = indexed_state(&context, &journal, logical_time, Oracle);
            let projected = project_query(&context, &journal, logical_time);
            assert_eq!(legacy, projected, "legacy parity at t_={ticks}");
        }
    }
}
